...
<p>No-op defaults are sufficient when setup/teardown are not needed.</p>
<div class="code-block">
<span class="code-block-label">No-op Resource impl</span>
<pre><code class="language-rust">struct WorkflowParams {
    batch_size: usize,
    timeout_ms: u64,
}
<!--blank-->
// Defaults to no-op setup() and teardown() — no #[async_trait] needed
impl Resource for WorkflowParams {}</code></pre>
</div>
</div>
<div class="card">
...
The default key type is <code>String</code>, which allows <code>&amp;str</code> ergonomics
via the <code>Borrow&lt;str&gt;</code> blanket impl.
</p>
...
<!--blank-->
// Plain parameter struct — no lifecycle needed
struct WorkflowParams {
    multiplier: u32,
}
<!--blank-->
// No #[async_trait] needed when both methods are no-ops
impl Resource for WorkflowParams {}
<!--blank-->
// Resource with non-trivial lifecycle
struct CounterResource {
    setup_count: Arc&lt;Mutex&lt;u32&gt;&gt;,
}
<!--blank-->
...
<td>Builder pattern — consumes and returns <code>self</code>. Duplicate keys trigger a <code>debug_assert!</code>. <code>#[must_use]</code>.</td>
</tr>
<tr>
