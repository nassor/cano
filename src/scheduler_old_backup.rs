...
//! the core scheduling functionality. Works with the new Workflow API.
//!
//! ## 🚀 Quick Start
//!
...
//!     let workflow4 = Workflow::new(store.clone());
//!     let workflow5 = Workflow::new(store.clone());
//!     let workflow6 = Workflow::new(store.clone());
//!     
//!     // Multiple ways to schedule workflows:
//!     scheduler.every_seconds("task1", workflow1, MyState::Start, 30)?;                    // Every 30 seconds
//!     scheduler.every_minutes("task2", workflow2, MyState::Start, 5)?;                     // Every 5 minutes  
//!     scheduler.every_hours("task3", workflow3, MyState::Start, 2)?;                       // Every 2 hours
//!     scheduler.every("task4", workflow4, MyState::Start, Duration::from_millis(500))?;    // Every 500ms
//!     scheduler.cron("task5", workflow5, MyState::Start, "0 */10 * * * *")?;              // Every 10 minutes (cron)
//!     scheduler.manual("task6", workflow6, MyState::Start)?;                               // Manual trigger only
//!     
