...
/// ```
#[async_trait]
pub trait Node<TState, TResourceKey = String>: Send + Sync
where
...
/// Type alias for a dynamic node trait object using the default `String` resource key.
///
/// Use this when you need to store different node types in the same collection.
pub type DynNode<TState> = dyn Node<TState, String, PrepResult = DefaultNodeResult, ExecResult = DefaultNodeResult>
    + Send
    + Sync;
...
pub type NodeObject<TState> = std::sync::Arc<DynNode<TState>>;
